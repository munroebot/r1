...
function _serializeDate(dateValue) {
	var dateParts = dateValue.split("/");

	if(dateFormat == 'mm/dd/yyyy') {
		serialDateArray = [dateParts[0],dateParts[1],dateParts[2]];
	}

	if(dateFormat == 'dd/mm/yyyy') {
		serialDateArray = [dateParts[1],dateParts[0],dateParts[2]];
	}

	if(dateFormat == 'yyyy/mm/dd') {
		serialDateArray = [dateParts[1],dateParts[2],dateParts[0]];
	}

	return serialDateArray
}
...
function drawCalendar() {
    ...
    if (!yyyy && !mm) {
        ...
        if (x.value != "") {
            ...
            var wholeValue = _serializeDate(x.value);

            var mm = wholeValue[0]*1;
            var dd = wholeValue[1]*1;
            var yyyy = wholeValue[2]*1;
        } else {
            var doDate = new Date();
            var mm = doDate.getMonth()+1;
            var dd = doDate.getDate();
...
function createCalendarWidget() {
    (arguments[0] ? idtag = arguments[0] : idtag = "");
    (arguments[1] ? dateFormat = arguments[1] : dateFormat = "mm/dd/yyyy");
    (arguments[2] ? isEditable = arguments[2] : isEditable = "EDIT");
    (arguments[3] ? hasIcon = arguments[3] : hasIcon = "NO_ICON");
    (arguments[4] ? iconPath = arguments[4] : hasIcon = "./OU812.gif");
